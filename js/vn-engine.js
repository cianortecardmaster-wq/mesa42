(function(){
  ...
  function setBg(bg){ document.getElementById('vnStage').style.setProperty('--scene-bg', `url('${asset(bg)}')`); }
  function setEffect(effect){
    const stage = $('#vnStage');
    stage.classList.remove('glitch','shake');
    $('#vnBg').classList.toggle('fade-black', effect === 'black');
    if(effect === 'glitch' || effect === 'shake') { void stage.offsetWidth; stage.classList.add(effect); }
  }
  ...
  function updateProgress(){
    ...
    let before = 0;
    for(const s of state.story.scenes){ if(s.id === state.sceneId) break; before += (s.lines?.length || 0); }
  ...
  function render(){
    const s = scene();
    if(!s) return;
    setBg(s.bg || state.story.defaultBg);
    renderCharacters(s.characters || []);
    const ln = line();
    if(!ln){ return goNextScene(); }
    $('#speaker').textContent = ln.speaker || 'Narrador';
  ...
  function renderChoices(choices){
    ...
    if(!choices || !choices.length){ wrap.hidden = true; $('#nextHint').hidden = false; return; }
    wrap.hidden = false; $('#nextHint').hidden = true;
    choices.forEach(choice => {
      const btn = document.createElement('button');
      btn.className = 'choice'; btn.type = 'button'; btn.textContent = choice.label;
  ...
  function choose(choice){
    ...
    if(choice.next){ state.visited.push(state.sceneId); state.sceneId = choice.next; state.lineIndex = 0; }
    else state.lineIndex++;
    render();
  }
  function goNextScene(){
    const idx = state.story.scenes.findIndex(s => s.id === state.sceneId);
    const cur = scene();
    if(cur?.next){ state.visited.push(state.sceneId); state.sceneId = cur.next; state.lineIndex = 0; render(); return; }
    if(idx >= 0 && idx < state.story.scenes.length - 1){ state.visited.push(state.sceneId); state.sceneId = state.story.scenes[idx+1].id; state.lineIndex = 0; render(); return; }
  ...
  function next(){
    ...
    const s = scene();
    if(state.lineIndex < (s.lines.length - 1)){ state.lineIndex++; render(); }
    else goNextScene();
  }
  ...
  function setupInvestigation(){
    document.querySelectorAll('[data-inspect]').forEach(btn => {
      btn.addEventListener('click', (ev) => {
        ...
        $('#speaker').textContent = texts[key]?.speaker || 'JP';
        $('#lineText').textContent = texts[key]?.text || 'Nada aqui parece normal.';
        state.flags[`inspect_${key}`] = true;
        const required = ['nome','topo','baixo','foil'];
        if(required.every(k => state.flags[`inspect_${k}`])){
          setTimeout(()=>{ $('#investigatePanel').classList.remove('open'); state.lineIndex++; render(); }, 650);
        }
      });
    });
  }
  ...
  async function restore(){
    ...
    if(saved?.scene_id){ state.sceneId = saved.scene_id; state.lineIndex = Number(saved.line_index || 0); state.flags = saved.flags || {}; }
  }
  async function init(opts){
    els.dialogueBox = $('#dialogueBox');
    state.slug = opts.storySlug; state.basePath = opts.basePath || '';
    const res = await fetch(opts.storyUrl); state.story = await res.json();
